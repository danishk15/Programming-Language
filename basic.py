...
class ExpectedCharError(Error):
    def __init__(self, pos_start, pos_end, details):
        super().__init__(pos_start, pos_end, 'Expected Charecter', details)
...
class position:
    def __init__(self,idx,ln,cln,fn,ftxt):
        self.idx=idx
        self.cln=cln
        self.ln=ln
        self.fn=fn
    ...
    def copy(self):
        return position(self.idx,self.ln,self.cln,self.fn,self.ftxt)
...
class Lexer:
    ...
    def make_string(self):
        ...
        while self.current_char is not None and (self.current_char != '"' or escape):
            if escape:
...
class IfNode:
    def __init__(self, cases, else_case):
        ...
        self.pos_end = (self.else_case[0].pos_end if self.else_case else self.cases[-1][1].pos_end)
...
class ParseResult:
    ...
    def failure(self,error):
        if not self.error or self.advance_count == 0:# OR HAVEN'T ADVANCED SINCE
            self.error=error
        return self
...
#############################
    ...
    def statements(self):
        ...
        if self.current_tok.type == TT_EOF or self.current_tok.matches(TT_KEYWORD, 'END') or self.current_tok.matches(TT_KEYWORD, 'ELIF') or self.current_tok.matches(TT_KEYWORD, 'ELSE'):
        ...
        while True:
            newline_count = 0
            while self.current_tok.type == TT_NEWLINE:
                res.register_advancement(); self.advance(); newline_count += 1
            if newline_count == 0:
                break
            if self.current_tok.type == TT_EOF or self.current_tok.matches(TT_KEYWORD, 'END') or self.current_tok.matches(TT_KEYWORD, 'ELIF') or self.current_tok.matches(TT_KEYWORD, 'ELSE'):
                break
    ...
    def statement(self):
        ...
        if res.error:
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start,self.current_tok.pos_end,
                " Expected ')' , 'VAR','RETURN','BREAK','CONTINUE', 'IF','FOR','WHILE','FUNC','int','float','identifier','+','-','(','[' or ''NOT "
            ))
    ...
    def if_expr_cases(self, case_keyword):
        ...
        if self.current_tok.matches(TT_KEYWORD, 'ELIF'):
            more_cases, else_case = res.register(self.if_expr_cases('ELIF'))
            if res.error: return res
            ...
            if self.current_tok.type == TT_NEWLINE:
                ...
                else_body = res.register(self.statement())
                if res.error: return res
                else_case = (else_body, False)

        if case_keyword == 'IF':
            if self.current_tok.matches(TT_KEYWORD, 'END'):
                res.register_advancement()
                self.advance()
            elif cases[-1][2] or else_case and else_case[1]:
                return res.failure(InvalidSyntaxError(
                    self.current_tok.pos_start, self.current_tok.pos_end,
                    "Expected 'END'"
                ))
        ...
        if self.current_tok.type == TT_LPAREN:
            ...
            if self.current_tok.type == TT_RPAREN:
                ...
                if res.error:
                    return res.failure(InvalidSyntaxError(
                        self.current_tok.pos_start,self.current_tok.pos_end,
                        f"Expected ')' , 'VAR','IF','FOR','WHILE','FUNC','int','float','identifier','+','-','(','[' or ''NOT "
                    ))
                ...
                res.register_advancement()
                self.advance()
                return res.success(CallNode(atom,args_nodes))
    ...
    def atom(self):
        ...
        return res.failure(InvalidSyntaxError(
            ...
            "Expected int or float ,'identifier','+','-',' or '(','[','IF','FOR','WHILE','ELSE'"
            ))
    ...
    def func_def(self):
        ...
        if not self.current_tok.matches(TT_KEYWORD, 'FUNC'):
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end, "Expected 'FUNC'"))
    ...
    def subbed_by(self, other):
        if isinstance(other,Number):
            ...
            try:
                ...
                return None, RunTimeError(
                    other.pos_start,other.pos_end,
                    'Element at this index could not be removed from list because index not exists or out of bounds',
                    self.context
                )
        else:
            return None,self.illegal_operation(other)
    ...
    def dived_by(self, other):
        if isinstance(other,Number):
            try:
                ...
                return None, RunTimeError(
                    other.pos_start,other.pos_end,
                    'Element at this index could not be retrieved from list because index not exists or out of bounds',
                    self.context
                )
        else:
            return None,self.illegal_operation(other)
...
class BaseFunction(Value):
    ...
    def check_args(self, args_names,args):
        res = RTResult()

        if len(args) > len(args_names):
            return res.failure(RunTimeError(
                self.pos_start, self.pos_end,
                f"{len(args)- len(args_names)} too many args passed into '{self.name}'",
                self.context
            ))
        ...
        if len(args) < len(args_names):
            return res.failure(RunTimeError(
                self.pos_start, self.pos_end,
                f"{len(args_names )- len(args)} too few args passed into '{self.name}'",
                self.context
            ))
...
class Function(BaseFunction):
        def __init__(self,name,body_node,args_names,should_auto_return):
            super().__init__(name)
            self.body_node = body_node
        ...
        def execute(self,args):
            ...
            res.register(self.check_and_populate_args(self.args_names,args,exec_ctx))
            if res.should_return():return res
        ...
        def copy(self):
            copy = Function(self.name , self.body_node,self.args_names,self.should_auto_return)
            copy.set_context(self.context)
            copy.set_pos(self.pos_start,self.pos_end)
            return copy
...
class BuiltInFunction(BaseFunction):
    ...
    def no_visit_method(self, exec_ctx):
        return RTResult().failure(RunTimeError(
            self.pos_start, self.pos_end,
            f"No built-in function '{self.name}' exists",
            exec_ctx
        ))
...
##############################
    ...
    def execute_input_int(self,exec_ctx):
        while True:
            text = input()
            try:
                return RTResult().success(Number(int(text)))
            except ValueError:
    ...
    def execute_pop(self,exec_ctx):
        ...
        if not isinstance(index,Number):
            return RTResult().failure(RunTimeError(
                self.pos_start,self.pos_end,
                "Second arguement must be number",
                exec_ctx
            ))
    ...
    def execute_run(self,exec_ctx):
        ...
        if error:
            return RTResult().failure(RunTimeError(
                self.pos_start,self.pos_end,
                f"failed to fetch executing script \"{fn}\"\n" + error.as_string(),
                exec_ctx
            ))
...
##############################
    ...
    def visit_ForNode(self,node,context):
        ...
        if not isinstance(start_value, Number) or not isinstance(end_value, Number) or not isinstance(step_value, Number):
            return res.failure(RunTimeError(
                node.pos_start, node.pos_end,
                'FOR loop bounds and STEP must be numbers', context))
        if step_value.value == 0:
            return res.failure(RunTimeError(
                node.pos_start, node.pos_end,
                'FOR loop STEP cannot be zero', context))
    ...
    def visit_FuncDefNode(self,node,context):
        ...
        func_value = Function(func_name,body_node,arg_name,node.should_auto_return).set_context(context).set_pos(node.pos_start,node.pos_end)
